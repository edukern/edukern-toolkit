...
import { signTenantToken, type TenantTokenOptions } from "./sign-tenant-token.js";
 ...
 */
export async function createTenantClient(tenantId: string, options: TenantClientOptions = {}): Promise<SupabaseClient> {
  ...
  if (!url || !anonKey) throw new Error(`${urlEnvVar} e ${anonKeyEnvVar} são obrigatórios`);
