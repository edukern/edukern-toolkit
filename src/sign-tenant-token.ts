...
function getSecret(secretEnvVar: string): Uint8Array {
  const cached = secretCache.get(secretEnvVar);
  if (cached) return cached;
  const s = process.env[secretEnvVar];
  if (!s) throw new Error(`${secretEnvVar} ausente — exigido pra assinar token de tenant`);
 ...
 * tenants).
 */
export async function signTenantToken(tenantId: string, options: TenantTokenOptions = {}): Promise<string> {
  if (!tenantId) {
