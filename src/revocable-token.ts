 ...
 */
export function hashOpaqueToken(token: string, secretEnvVar: string): string {
  const secret = process.env[secretEnvVar];
  if (!secret || secret.length < 16) throw new Error(`${secretEnvVar} ausente ou curto demais`);
