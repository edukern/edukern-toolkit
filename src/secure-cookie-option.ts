export type RequestLike = { headers: { get(name: string): string | null }; protocol?: string };

function isHttpsRequest(headers: { get(name: string): string | null }, protocol?: string): boolean {
 ...
 */
export function resolveSecureCookieOption(request?: RequestLike): boolean {
  const override = process.env.AUTH_COOKIE_SECURE?.trim().toLowerCase();
  if (override === "true") return true;
  if (override === "false") return false;
  if (request) return isHttpsRequest(request.headers, request.protocol);
