...
test("payload é TLV bem formado terminando em 63 + CRC de 4 hex", () => {
  ...
  assert.match(
    payload,
    /^00020101021126330014BR\.GOV\.BCB\.PIX0111111444777355204000053039865802BR5913Fulano de Tal6008BRASILIA62070503\*\*\*6304[0-9A-F]{4}$/
  );
});
...
test("CRC é autoconsistente (recalculado bate com o final do payload)", () => {
  ...
  for (let i = 0; i < withoutCrc.length; i++) {
    ...
    for (let bit = 0; bit < 8; bit++) acc = acc & 0x8000 ? ((acc << 1) ^ 0x1021) & 0xffff : (acc << 1) & 0xffff;
  }
