...
test("retorna o endereço quando o CEP existe", async () => {
  globalThis.fetch = (async () =>
    new Response(
      JSON.stringify({ cep: "01310-100", logradouro: "Avenida Paulista", bairro: "Bela Vista", localidade: "São Paulo", uf: "SP" })
    )) as typeof fetch;
...
test("retorna null quando o ViaCEP responde {erro: true}", async () => {
  globalThis.fetch = (async () => new Response(JSON.stringify({ erro: true }))) as typeof fetch;
