 ...
 */
export function buildPixCopyPaste(charge) {
    const { pixKey, merchantName, merchantCity, amount, txid = "***" } = charge;
    if (!pixKey)
        throw new Error("buildPixCopyPaste() exige pixKey");
    if (!merchantName)
        throw new Error("buildPixCopyPaste() exige merchantName");
    if (!merchantCity)
        throw new Error("buildPixCopyPaste() exige merchantCity");
    const merchantAccountInfo = tlv("00", "BR.GOV.BCB.PIX") + tlv("01", pixKey);
    const withoutCrc = tlv("00", "01") +
        tlv("01", "11") +
